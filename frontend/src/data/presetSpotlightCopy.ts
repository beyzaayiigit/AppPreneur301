/** Haftalık vitrin (indeks 1–15, Original hariç) — her ön ayara özel kısa metin. */

export const PRESET_SPOTLIGHT_BODY: Record<number, string> = {
...
export function spotlightBodyForPresetIndex(index: number, fallbackTitle: string): string {
  return PRESET_SPOTLIGHT_BODY[index] ?? `${fallbackTitle} — Looks sekmesinde deneyin; yoğunlukla ince ayar yapın.`;
}
